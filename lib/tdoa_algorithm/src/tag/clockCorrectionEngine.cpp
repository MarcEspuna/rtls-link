...
#define MAX_CLOCK_DEVIATION_SPEC_FACTOR 5     // TODO: This has been added in order to not discard packets due clock dev TODO: Look into why
#define MAX_CLOCK_DEVIATION_SPEC 10e-6

#define CLOCK_CORRECTION_SPEC_MIN (1.0 - MAX_CLOCK_DEVIATION_SPEC * MAX_CLOCK_DEVIATION_SPEC_FACTOR)
#define CLOCK_CORRECTION_SPEC_MAX (1.0 + MAX_CLOCK_DEVIATION_SPEC * MAX_CLOCK_DEVIATION_SPEC_FACTOR)
...
bool clockCorrectionEngineUpdate(clockCorrectionStorage_t* storage, const double clockCorrectionCandidate) {
  ...
  if (-CLOCK_CORRECTION_ACCEPTED_NOISE < difference && difference < CLOCK_CORRECTION_ACCEPTED_NOISE) {
    ...
    // printf("Clock correction too much noise\n");
    ...
    if (shouldAcceptANewClockReference) {
      if (CLOCK_CORRECTION_SPEC_MIN < clockCorrectionCandidate && clockCorrectionCandidate < CLOCK_CORRECTION_SPEC_MAX) {
        ...
        // printf("New clock correction candidate\n");
      } else {
        // printf("clk corr disc %f, min %f, max %f\n", clockCorrectionCandidate, CLOCK_CORRECTION_SPEC_MIN, CLOCK_CORRECTION_SPEC_MAX);
      }
    }
  }
